(function(global,undefined){
	...
	global.extend({
		View: function(name,onrender,onopen,onclose){
			if(views[name]){
				throw new Error('View already exists.');
			}
			if(!onopen){
				throw new Error('Parameter onopen is required.');
			}
			if(!onrender){
				throw new Error('Parameter onrender is required.');
			}
			if(!onclose){
				throw new Error('Parameter onclose is required.');
			}
			var parent,
				open = false,
				self = this.extend({
					...
					open: function(selector){
						...
						dom.get(parent).children = onrender();
						onopen();
						return this;
					},
					close: function(){
